    ...
    def execute(self, input_data:UpdateStudentInput) -> UpdateStudentOuput:
        ...
        if input_data.faculty:
            if student._in_prep():
                ...
                FACULTY_M = f"Faculty has been updated to {input_data.faculty.value} successfully"
            else:
                FACULTY_M = f"Cannot change Faculty after {Grade.PREP.value}"

        if input_data.grade:
            if student.validateGrade(input_data.grade):
                student.grade = input_data.grade
                GRADE_M = f"Grade has been updated to {input_data.grade.value} successfully"
            else: 
                GRADE_M = f"Student with ID:{input_data.id} cannot raise in {input_data.grade.value}"

        if input_data.gpa:
            student.gpa = input_data.gpa
            GPA_M = f"GPA has been updated to {input_data.gpa} successfully"
